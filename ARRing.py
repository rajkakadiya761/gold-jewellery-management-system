...
FINGER_X_OFFSET = 0  # Horizontal offset from finger 
FINGER_Y_OFFSET = 100.80  # Vertical offset from finger 
JEWELLERY_SCALE = 0.8 # Scale factor for the jewellery size 
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                x_finger = int(finger_tip.x * WIDTH)
                y_finger = int(finger_tip.y * HEIGHT)
                ...
                y_offset = int(y_finger + FINGER_Y_OFFSET - jewellery_height / 2)
                x_offset = int(x_finger + FINGER_X_OFFSET - jewellery_width / 2)

                print(f"Finger Tip Coordinates: x={x_finger}, y={y_finger}")
                print(f"Jewellery Position: x_offset={x_offset}, y_offset={y_offset}")
                ...
                if x_offset < 0 or y_offset < 0 or x_offset + jewellery_width > WIDTH or y_offset + jewellery_height > HEIGHT:
                    print("Warning: Jewellery is out of bounds. Adjusting offsets.")
                    continue
        ...
        # Display exit message on the camera screen
